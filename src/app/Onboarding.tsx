...
import {
  Heart, X, CheckCircle, AlertCircle, CreditCard, FileText, Briefcase,
} from "lucide-react";
...
function UploadBtn({ label, sub, uploaded, onUpload }: { label: string; sub: string; uploaded: boolean; onUpload: () => void }) {
  return (
    <button type="button" onClick={onUpload} className={`w-full flex items-center gap-3 p-4 rounded-xl border-2 text-left transition-all ${uploaded ? "border-emerald-300 bg-emerald-50" : "border-dashed border-slate-200 hover:border-[#0EA5A0]/50 hover:bg-[#F2F5FA]"}`}>
      <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${uploaded ? "bg-emerald-100" : "bg-[#E8EEF8]"}`}>
        {uploaded ? <CheckCircle size={17} className="text-emerald-600" /> : <FileText size={17} className="text-[#1B3A6B]" />}
      </div>
      <div>
        <p className={`text-sm font-semibold ${uploaded ? "text-emerald-700" : "text-slate-800"}`}>{label}</p>
        <p className="text-xs text-slate-400 mt-0.5">{uploaded ? "Uploaded successfully ✓" : sub}</p>
      </div>
    </button>
  );
}
...
export function Onboarding({ initialMode, initialRole, onSuccess, onBack }: {
  ...
  const [cg, setCg] = useState({
    ...
    govId: false, pswCert: false, vsc: false, firstAid: false,
  ...
  const finishCaregiver = async () => {
    ...
    await supabase.from("profiles").upsert({ id: uid, email, full_name: full, role: "caregiver", verified: false, suspended: false, hourly_rate: Number(cg.hourlyRate), city: cg.cities[0] ?? "" });
        ...
        <div className="bg-white rounded-3xl border border-[rgba(15,23,42,0.10)] shadow-sm p-8">
          ...
          {stage === "f3" && (
            <div className="space-y-6">
              ...
              <div className="bg-[#F2F5FA] rounded-2xl p-5 border border-[rgba(15,23,42,0.10)]">
                ...
                {!fm.vettingPaid
                  ? <button type="button" onClick={() => fm_("vettingPaid", true)} className="w-full py-3 bg-[#0EA5A0] text-white rounded-xl font-bold text-sm hover:bg-[#0d9489] transition-colors flex items-center justify-center gap-2"><CreditCard size={15} />Pay $39.00 &amp; Activate Account</button>
          ...
          {stage === "cg3" && (
            <div className="space-y-4">
              ...
              <UploadBtn label="Government-Issued Photo ID" sub="Ontario Driver's Licence or Canadian Passport · PDF, JPG, PNG" uploaded={cg.govId} onUpload={() => cg_("govId", true)} />
              {cg.pswRole === "psw" && <UploadBtn label="Proof of Qualifications (Certified PSW)" sub="Ontario College Certificate, Diploma, or proof of nursing registration" uploaded={cg.pswCert} onUpload={() => cg_("pswCert", true)} />}
              <UploadBtn label="Vulnerable Sector Check (VSC)" sub="Valid Ontario police check issued within the last 12 months" uploaded={cg.vsc} onUpload={() => cg_("vsc", true)} />
              <UploadBtn label="First Aid / CPR Certification" sub="Active St. John Ambulance or Red Cross certificate (recommended)" uploaded={cg.firstAid} onUpload={() => cg_("firstAid", true)} />
